...
``pandas`` is installed (``pip install 'aqwel-aion[ai]'``); otherwise CSV, JSON,
and JSONL work with the standard library only.

Examples
--------
>>> from aion.datasets import read_csv, read_file
...
def _require_pandas(fmt: str) -> None:
    if not _has_pandas():
        raise ImportError(
            f"Reading {fmt!r} requires pandas. Install with: pip install 'aqwel-aion[ai]'"
        )
...
def read_file(
    ...
    **kwargs: Any,
    ...
    With ``pip install 'aqwel-aion[ai]'``: ``.parquet``, ``.xlsx``, ``.xls``,
