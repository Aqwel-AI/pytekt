"""Optional Gradio / Streamlit app launchers (``pip install 'aqwel-aion[ui]'``)."""
...
def launch_gradio_playground(
    *,
    ...
    Requires: ``pip install 'aqwel-aion[ui]'``
    """
    if not gradio_available():
        raise ImportError(
            "Gradio is not installed. Install with: pip install 'aqwel-aion[ui]'"
        )
    import gradio as gr
...
def launch_streamlit_dataset_explorer(
    *,
    ...
    if not streamlit_available():
        raise ImportError(
            "Streamlit is not installed. Install with: pip install 'aqwel-aion[ui]'"
        )
    import subprocess
    import sys
...
from aion.datasets import fetch, list_datasets
