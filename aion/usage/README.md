...
Agent `/usage` slash command: **not available** (no `aion agent` in 0.2.0). Use `aion usage` today.
...
Requires for full hardware stats: `pip install 'aqwel-aion[monitor]'` (psutil; optional NVIDIA GPU)

## React dev mode (hot reload)

```bash
# Terminal 1 — API server
python3 -m aion.cli usage --no-browser
