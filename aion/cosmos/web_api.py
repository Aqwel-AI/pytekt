...
def library_info() -> Dict[str, Any]:
    try:
        ...
        return {"app": "cosmos", "version": "unknown"}
...
def get_observer_config() -> Dict[str, float]:
    try:
        ...
        cfg = get_config()
        cosmos = cfg.get("cosmos") or {}
        return {
            "latitude": float(cosmos.get("latitude", 40.18)),
            "longitude": float(cosmos.get("longitude", 44.51)),
        }
    except Exception:
...
def save_observer_config(latitude: float, longitude: float) -> Dict[str, float]:
    ...
    cfg = get_config()
    cfg.setdefault("cosmos", {})
    cfg["cosmos"]["latitude"] = latitude
    cfg["cosmos"]["longitude"] = longitude
    save_config(cfg)
