...
def is_pytekt_universe_server(host: str, port: int) -> bool:
...
def _start_server_thread(host: str, port: int) -> None:
    ...
    def _run():
        from pytekt.universe.service.server import run_server
    ...
    *,
    ...
    with _SERVER_LOCK:
        if _SERVER_THREAD is not None and _SERVER_THREAD.is_alive() and _SERVER_PORT:
            if is_pytekt_universe_server(host, _SERVER_PORT):
    ...
    if started_new and wait_ready > 0:
        ...
        while time.time() < deadline:
            if is_pytekt_universe_server(host, use_port):
                break
            time.sleep(0.08)
