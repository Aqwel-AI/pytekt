...
from pytekt.universe.core.constants import AU, LIGHT_YEAR, PARSEC
...
def flux_to_magnitude(flux: float, flux_zero: float = 1.0) -> float:
    ...
    from pytekt.universe.core._native import flux_to_magnitude as _ftm

    return _ftm(flux, flux_zero)


def magnitude_to_flux(magnitude: float, flux_zero: float = 1.0) -> float:
    from pytekt.universe.core._native import magnitude_to_flux as _mtf

    return _mtf(magnitude, flux_zero)
