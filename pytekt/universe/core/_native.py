...
def gmst_hours(jd: float) -> float:
    ...
    from pytekt.universe.core.time import _gmst_py
...
def equatorial_to_horizontal(
    ...
    from pytekt.universe.astrometry import coordinates
...
def ecliptic_to_equatorial(lon_deg: float, lat_deg: float = 0.0) -> Tuple[float, float]:
    if _NATIVE_AVAILABLE:
        ...
        return float(ra), float(dec)
    from pytekt.universe.ephemeris import observing
...
def moon_phase_fraction(jd: float) -> float:
    ...
    from pytekt.universe.ephemeris.observing import _moon_phase_fraction_py as _py
...
def comoving_distance_mpc(
    ...
    from pytekt.universe.cosmology.cosmology import _comoving_distance_py
...
def lookback_time_gyr(
    ...
    from pytekt.universe.cosmology.cosmology import _lookback_time_py
...
def redshift_from_velocity(v_kms: float) -> float:
    ...
    from pytekt.universe.cosmology.cosmology import _redshift_from_velocity_py
...
def true_anomaly_from_mean(M_deg: float, e: float, *, tol: float = 1e-8) -> float:
    ...
    from pytekt.universe.ephemeris import orbits
