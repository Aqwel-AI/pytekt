...
from pytekt.universe.astrometry.coordinates import equatorial_to_horizontal
from pytekt.universe.core.time import gmst, julian_centuries, now_jd
from pytekt.universe.core.units import deg_to_rad, rad_to_deg
...
def moon_phase(jd: float) -> Tuple[float, str]:
    ...
    """
    from pytekt.universe.core._native import moon_phase_fraction
...
def moon_illumination(jd: float) -> float:
    from pytekt.universe.core._native import moon_illumination as _moon_illum
...
def air_mass(altitude_deg: float, *, pickering: bool = False) -> float:
    ...
    from pytekt.universe.core._native import air_mass as _air_mass_native

    return _air_mass_native(altitude_deg, pickering=pickering)


def is_circumpolar(dec_deg: float, latitude_deg: float) -> bool:
    from pytekt.universe.core._native import is_circumpolar as _is_circ
...
def rise_set_approx(
    ...
    """
    from pytekt.universe.core._native import rise_set_approx as _rise_set
...
def ecliptic_to_equatorial(lon_deg: float, lat_deg: float = 0.0) -> Tuple[float, float]:
    ...
    from pytekt.universe.core._native import ecliptic_to_equatorial as _ecl_eq
...
def planet_catalog_entries(jd: float) -> List[Dict[str, Any]]:
    ...
    from pytekt.universe.catalogs.catalogs import load_planets
    from pytekt.universe.ephemeris.orbits import planet_position
...
def build_sky_catalog(catalog_mode: str = "stars", jd: Optional[float] = None) -> List[Dict[str, Any]]:
    ...
    from pytekt.universe.catalogs.catalogs import load_bright_stars, load_messier
...
def whats_up(
    ...
    *,
    ...
    from pytekt.universe.catalogs.catalogs import load_bright_stars
    ...
    if not objects:
        return []
    import numpy as np
    from pytekt.universe.core._native import equatorial_to_horizontal_batch
