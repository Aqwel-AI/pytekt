# pytekt.monitor — Package documentation
...
| `launch.py` | `run_monitor_command` — spawns `python -m uvicorn pytekt.monitor.server:app`, optional browser. |
| `cli.py` | `python -m pytekt.monitor.cli` entry (argparse); same behavior as `pytekt monitor`. |
...
python3 -m pytekt dashboard            # alias of monitor
python3 -m pytekt monitor --no-browser # print URL only
python3 -m pytekt monitor --docs       # open Swagger UI instead of dashboard
python3 -m pytekt monitor -p 8765      # custom port
```

Equivalent module entry:

```bash
python3 -m pytekt.monitor.cli --help
```

### Uvicorn only

```bash
python3 -m uvicorn pytekt.monitor.server:app --host 127.0.0.1 --port 8000
```
...
```python
from pytekt.monitor.hardware import get_cpu, get_ram, get_cpu_detailed
from pytekt.monitor.gpu import get_gpu_metrics
from pytekt.monitor.processes import top_memory_processes
...
For the **full snapshot** (history + processes) as served to the dashboard, start the app so the **sampler** runs, then `GET /api/metrics/snapshot` or use **`pytekt.monitor.history.sampler.snapshot()`** only after the server lifespan has started the sampler (normally via `server.py`).
...
```bash
python -m pytekt.monitor.examples.demo_snapshot
```
...
- Root CLI: **`pytekt.cli`** (`python3 -m pytekt --help`)
