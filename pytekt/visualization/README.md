...
If you install with the `[viz]` or `[full]` extra, these are installed automatically. If you install only the base package (`pip install pytekt`), you must install matplotlib and numpy yourself before using `pytekt.visualization`.
...
3. **Verify the visualization module is importable.**
   ```bash
   python -c "from pytekt.visualization import plot_array; from pytekt.visualization.utils import save_plot; print('OK')"
   ```
...
```python
from pytekt.visualization import plot_array
...
- Import plot functions from `pytekt.visualization`:
  ```python
  from pytekt.visualization import plot_array, plot_histogram, plot_matrix_heatmap, plot_training_history
  ```
...
Import from `pytekt.visualization` when seaborn is installed. All functions return a matplotlib `Figure` (or clustermap grid) and accept `title` / `show`.

| Function | Description |
|----------|-------------|
| `set_pytekt_style()` | Apply dark research theme aligned with PyTekt UI palette. |
...
Shared helpers (import from `pytekt.visualization.utils`).
...
- **Import pattern**: `from pytekt.visualization import plot_array, plot_histogram, ...` and `from pytekt.visualization.utils import save_plot`.
...
1. Import the plot functions and `save_plot`:
   ```python
   from pytekt.visualization import plot_array, plot_histogram
   from pytekt.visualization.utils import save_plot
   ```
...
1. Import the functions you need (you usually do not need `save_plot` unless you also want to export files):
   ```python
   from pytekt.visualization import plot_array, plot_training_history
   ```
...
4. Example structure:
   ```python
   from pytekt.visualization import plot_array, plot_histogram, plot_matrix_heatmap
   from pytekt.visualization.utils import save_plot
...
```python
from pytekt.visualization import plot_training_history
from pytekt.visualization.utils import save_plot
...
```python
from pytekt.visualization import plot_confusion_matrix
from pytekt.visualization.utils import save_plot
...
```python
from pytekt.visualization import plot_scatter_with_fit
from pytekt.visualization.utils import save_plot
...
```python
from pytekt.visualization import plot_train_vs_val
from pytekt.visualization.utils import save_plot
