...
**Not** the old HTML chat on port 3333 — that is usually another app (PHP). PyTekt uses **port 3847** by default.

## Run (recommended)

```bash
# From repo root — builds React + opens browser
./pytekt/usage/run_dashboard.sh

# Or if pytekt is installed:
pytekt usage
```

Open: **http://127.0.0.1:3847/** (not port 3333 — that may be another app)

Agent `/usage` slash command: **not available** (no `pytekt agent` in 0.2.0). Use `pytekt usage` today.
...
cd pytekt/usage/web && npm install && npm run dev
```
...
```bash
cd pytekt/usage/web && npm install && npm run build
```

Output: `pytekt/usage/static/` (served by Python).
