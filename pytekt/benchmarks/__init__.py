#!/usr/bin/env python3
"""
Benchmarking, timing, and reproducibility utilities for PyTekt.
"""
...
def timed_run(fn: Callable[..., Any], *args: Any, repeats: int = 1, **kwargs: Any) -> Dict[str, Any]:
    """
    Time the execution of a function over multiple repeats.

    Returns a dict containing mean_seconds, min_seconds, max_seconds, repeats, and last result.
    """
    times: List[float] = []
    res: Any = None
...
def compare_sum_numpy_vs_fast(arr: Any, repeats: int = 3) -> Dict[str, Any]:
    """
    Compare NumPy sum vs PyTekt fast_sum performance.
    """
    ...
    try:
        from .. import native
        using_native = native.core_status().available
    except Exception:
        pass

    t_numpy = timed_run(lambda: np.sum(arr), repeats=repeats)

    try:
        from .._core import fast_sum  # type: ignore
        t_fast = timed_run(lambda: fast_sum(arr), repeats=repeats)
    except Exception:
        t_fast = timed_run(lambda: float(sum(arr.tolist())), repeats=repeats)

    return {
        "numpy_mean_s": t_numpy["mean_seconds"],
        "fast_mean_s": t_fast["mean_seconds"],
        "using_native_extension": using_native,
        "repeats": repeats,
    }
...
def _timeit(fn: Callable[..., Any], runs: int = 3) -> BenchmarkResult:
    times: List[float] = []
    for _ in range(runs):
