...
def create_module_dependency_doc(
    ...
    for mod in modules:
        try:
            ...
            dep_map[mod] = []
            continue
        deps = _get_pytekt_imports_from_source(path)
        dep_map[mod] = deps
        for d in deps:
            if d != mod:
...
similar = embed.find_similar_texts("Artificial intelligence", corpus)
        ...
        ("Model Evaluation", """
import pytekt.evaluate as evaluate
