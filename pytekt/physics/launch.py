...
def is_pytekt_physics_server(host: str, port: int) -> bool:
...
def resolve_physics_port(host: str, preferred: int = _DEFAULT_PORT) -> Tuple[int, bool]:
    if is_pytekt_physics_server(host, preferred):
        return preferred, True
    if not _port_open(host, preferred):
        return preferred, False
    for offset in range(1, _PORT_SCAN_RANGE):
        candidate = preferred + offset
        if is_pytekt_physics_server(host, candidate):
    ...
    *,
    ...
    with _SERVER_LOCK:
        if _SERVER_THREAD is not None and _SERVER_THREAD.is_alive() and _SERVER_PORT:
            if is_pytekt_physics_server(host, _SERVER_PORT):
    ...
    if started_new and wait_ready > 0:
        ...
        while time.time() < deadline:
            if is_pytekt_physics_server(host, use_port):
                break
            time.sleep(0.08)
