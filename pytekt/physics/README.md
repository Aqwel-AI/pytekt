...
```bash
pytekt physics query "kinetic energy mass=2 velocity=3"
pytekt physics pendulum --length 1 --angle-deg 15
pytekt physics projectile --v0 20 --angle 45
pytekt physics tasks
pytekt physics web              # browser dashboard (port 3858)
pytekt physics-dashboard
```
...
Optional config in `~/.pytekt.yaml`:
...
- `rk4_step`, `integrate_trajectory_rk4`, `pendulum_trajectory` via `pytekt._pytekt_physics`
