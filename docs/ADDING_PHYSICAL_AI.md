...
| Agent slash commands | `aion/cli_agent/universe_cmds.py` |
...
## Step 8 — Agent integration (`aion agent`)

Physical AI is most useful when the **terminal agent** can call physics safely.

### 8.1 Slash commands

Create `aion/cli_agent/physics_cmds.py` (mirror `universe_cmds.py`):
...
Register in `aion/cli_agent/app.py` where `/sky` is wired to `universe_cmds.handle_sky_command`.

### 8.2 Agent tools (recommended)

In `aion/cli_agent/tools.py` (or a small registry file), add tools that wrap your API:
...
- [Terminal agent README](../aion/cli_agent/README.md)
